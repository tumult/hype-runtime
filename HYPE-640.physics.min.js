...
Matter.HypePlugin = {
	//!! cannot change signature since this is not versioned!
	fromVerticesWithFallback: function(x, y, vertexSets, options, flagInternal, removeCollinear, minimumArea, removeDuplicatePoints, isVectorShape, shouldTryConvexHull, alwaysUseConvexHull) {
		
		var isPhysicsBodySane = function (physicsBody, convexHullPhysicsBody) {
			if(physicsBody.parts == null || physicsBody.parts.length < 1) {
				return true;
			}
			
			if(physicsBody.parts.length < 2) { // no need to check self-intersection if only one part
				return true;
			}

			// A. Make sure the center of each part is within the shape
			if(convexHullPhysicsBody != null) {
				var convexHullTriangles = triangulatePart(convexHullPhysicsBody);
				for(var i = 0; i < physicsBody.parts.length; i++) {
					var part = physicsBody.parts[i];
					var pointFoundWithinTriangle = false;
					
					for(var j = 0; j < convexHullTriangles.length; j++) {
						if(pointInTriangle(part.position, convexHullTriangles[j]) == true) {
							pointFoundWithinTriangle = true;
							break;
						}
					}
					
					if(pointFoundWithinTriangle == false) {
						return false;
					}
				}
			}
			...
			// B. Check for issue where parts can self-intersect (they shoudl never be able to)
				// 1: use fan triangulation on each part
				// 2: see if each triangle from each part intersects a triangle from another part
					// each triangle is 3 lines, see if nay intersect or if any verteces are within the triangle
					// see if any points are within a triangle
					// https://stackoverflow.com/questions/1585459/whats-the-most-efficient-way-to-detect-triangle-triangle-intersections

			var partTriangles = [];
			for(var i = 0; i < physicsBody.parts.length; i++) {
				var part = physicsBody.parts[i];
				if(part == physicsBody || part.vertices.length < 3) {
					continue;
				}
				
				partTriangles.push(triangulatePart(part));
			}
			
			return compareTriangleLists(partTriangles, partTriangles);
		};

		var compareTriangleLists = function (triangleList1, triangleList2) {
			for(var i = 0; i < triangleList1.length; i++) {
				for(var j = 0; j < triangleList2.length; j++) {
					var triangles1 = triangleList1[i];
					var triangles2 = triangleList2[j];
					
					if(triangles1 == triangles2) {
						continue;
					}
					
					for(var k = 0; k < triangles1.length; k++) {
						for(var l = 0; l < triangles2.length; l++) {
							var triangle1 = triangles1[k];
							var triangle2 = triangles2[l];
							if(trianglesIntersect(triangle1, triangle2) == true) {
								return false;
							}
						}
					}
		...
		var triangulatePart = function (physicsPart) {
			// assumes convex hull; uses fan method
			var triangles = [];
			var vertex1 = physicsPart.vertices[0];
			for(var j = 0; j < physicsPart.vertices.length - 2; j++) {
				var vertex2 = physicsPart.vertices[j + 1];
				var vertex3 = physicsPart.vertices[j + 2];
				var triangle = [vertex1, vertex2, vertex3];
				triangles.push(triangle);
			}
			return triangles;
		};

		var trianglesIntersect = function (triangle1, triangle2) {
			// from https://stackoverflow.com/a/23356273
			
			var linesToCheck = [
				[triangle1[0],triangle1[1],triangle2[0],triangle2[1]],
				[triangle1[0],triangle1[1],triangle2[0],triangle2[2]],
				[triangle1[0],triangle1[1],triangle2[1],triangle2[2]],
				[triangle1[0],triangle1[2],triangle2[0],triangle2[1]],
				[triangle1[0],triangle1[2],triangle2[0],triangle2[2]],
				[triangle1[0],triangle1[2],triangle2[1],triangle2[2]],
				[triangle1[1],triangle1[2],triangle2[0],triangle2[1]],
				[triangle1[1],triangle1[2],triangle2[0],triangle2[2]],
				[triangle1[1],triangle1[2],triangle2[1],triangle2[2]]];
			
			for(var i = 0; i < linesToCheck.length; i++) {
				var lineArgs = linesToCheck[i];
				if(linesIntersect(lineArgs[0], lineArgs[1], lineArgs[2], lineArgs[3])) {
					return true;
				}
			}
			
			return ((pointInTriangle(triangle2[0], triangle1) &&
					 pointInTriangle(triangle2[1], triangle1) &&
					 pointInTriangle(triangle2[2], triangle1)) ||
					(pointInTriangle(triangle1[0], triangle2) &&
					 pointInTriangle(triangle1[1], triangle2) &&
					 pointInTriangle(triangle1[2], triangle2)));
		};

		var linesIntersect = function (point1, point2, point3, point4) {
			var d = (point4.y-point3.y)*(point2.x-point1.x)-(point4.x-point3.x)*(point2.y-point1.y);
			var u = (point4.x-point3.x)*(point1.y-point3.y)-(point4.y-point3.y)*(point1.x-point3.x);
			var v = (point2.x-point1.x)*(point1.y-point3.y)-(point2.y-point1.y)*(point1.x-point3.x);
			...
			// avoid floating point precision issues by using tolerances
			if((u - 0 > .005  && d - u > .005 && v - 0 > .005  && d - v > .005) == true) {
				return true;
			}
			return false;
		};

		var pointInTriangle = function (point, triangle) {
		...
		if(isVectorShape == false || (alwaysUseConvexHull == false && shouldTryConvexHull == true)) {
			physicsBody = Matter.Bodies.fromVertices(x, y, vertexSets, options, flagInternal, removeCollinear, minimumArea, removeDuplicatePoints);
		}
		
		var centerPoint = {x:x, y:y};
		
		// make sure a vector shape results in a reasonably body, as poly-decomp can give weird results for self intersecting paths (#14973)
		if(isVectorShape == true) {
			var convexHullPhysicsBody;
			var convexHullCenterPoint;
			var convexHullVertices;
			
			if(shouldTryConvexHull == true) {
				convexHullVertices = Matter.Vertices.hull(vertexSets.slice(0));
				convexHullCenterPoint = Matter.Vertices.centre(convexHullVertices);
				convexHullPhysicsBody = Matter.Bodies.fromVertices(convexHullCenterPoint.x, convexHullCenterPoint.y, convexHullVertices, options, flagInternal, removeCollinear, minimumArea, removeDuplicatePoints);
			}
			
			if(physicsBody == null || isPhysicsBodySane(physicsBody, convexHullPhysicsBody) == false) {
				if(window['HYPE_DEBUG_PHYSICS'] == true) {
					console.log("error making concave shape for " + options.elementId + ", using convex hull");
				}

				physicsBody = convexHullPhysicsBody;
				vertices = convexHullVertices;
				centerPoint = convexHullCenterPoint;
			}
		}
		
		//!! cannot change signature since this is not versioned!
		return { "physicsBody" : physicsBody, "centerPoint" : centerPoint };
	}
};

(function () {
  var documentsToLoad = [];
  if(window['HYPE_pdtl_640'] != null) {
	documentsToLoad = window['HYPE_pdtl_640'].slice(0); // clone array
  }
  window['HYPE_pdtl_640'] = null;
